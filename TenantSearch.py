...
if zip_RTS is not None:
    ...
    df_Tenant=df_Tenant.assign(Point=(df_Tenant=='●').sum(axis=1)).sort_values(['Point','Tenant'],ascending=False).reset_index(drop=True)
    ...
    with st.expander(label='Property Tenant Data',expanded=True):
        ...
        ].sort_values('Tenant',ascending=False).reset_index(drop=True),height=500,use_container_width=True)
